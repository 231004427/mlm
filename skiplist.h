...
static struct skipnode *skipnode_new(int level, int key, VALUE_TYPE value)
{
...
skiplist_insert(struct skiplist *list, int key, VALUE_TYPE value)
{
        struct skipnode *nd;
        int rank[MAX_LEVEL];
...
static void skiplist_dump(struct skiplist *list)
{
    ...
    for (; i >= 0; i--) {
        traversed = 0;
        pos = pos->next;
        err_msg("pos->next:%p",pos);
        printf("level %d:\n", i + 1);
        //for (; pos != end; pos = pos->next)
        skiplist_foreach_forward(pos, end) {
            node = list_entry(pos, struct skipnode, link[i]);
            traversed += node->link[i].span;
            printf("key:%d value:%p rank:%u\n",
...
{
    //err_msg("skiplist_search_by_key %d key:%d",list->count,key);
        int i = list->level - 1;
        struct sk_link *pos = &list->head[i];
        struct sk_link *end = &list->head[i];
        struct skipnode *node;
